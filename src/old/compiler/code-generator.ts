...
export class CodeGenerator extends ASTVisitorBase {
    ...
    constructor(
        ...
        generateCode: (str: string) => void = (str) => process.stdout.write(str)
    ) {
        super();
        this._ast = ast;
    ...
    private generateFunction(node: ASTNode) {
        ...
        if (isMainEntry) {
            ...
            this.instruction(
                'add',
                `${RegisterAllocator.RSP.name},${RegisterAllocator.RBP.name},R0`
            );
        } else {
    ...
    private generateReturnStatement(node?: ASTNode) {
        ...
        if (localStackSize > 0) {
            ...
            this.instruction(
                'addi',
                `${RegisterAllocator.RSP.name},${RegisterAllocator.RSP.name},${localStackSize}`
            );
            this.line();
        }
        ...
        if (paramStackSize > 0) {
            ...
            this.instruction(
                'addi',
                `${RegisterAllocator.RSP.name},${RegisterAllocator.RSP.name},${paramStackSize}`
            );
            this.line();
        }
    ...
    private generateExpression(node: ASTNode): Address {
        switch (node.type) {
            ...
            case 'BinaryExpression': {
                ...
                this.instruction(
                    CodeGenerator.getBinaryInstruction(node.operator),
    ...
    private generateIdentifierExpression(
        node: ASTNode,
        previousType?: IntermediateType
    ): IntermediateType {
        ...
        if (previousType) {
            ...
            if (varEntry.type === 'data') {
                ...
                this.instruction(
                    'lw',
                    `${register.name},${thisEntry.offset}(${RegisterAllocator.RBP.name})`
                );
                this.instruction(
                    'addi',
                    `${register.name},${register.name},${this.getDataOffset(varEntry)}`
                );
                ...
                this.instruction(
                    'lw',
                    `${register.name},${varEntry.offset}(${RegisterAllocator.RBP.name})`
                );
    ...
    private generateMemberIndexExpression(
        node: ASTNode,
        previousType?: IntermediateType
    ): IntermediateType {
        const varEntry = node.symbolEntry;
        ...
        if (previousType) {
            ...
            switch (previousType.address.type) {
                ...
                case 'stackTop':
                    const stackRegister =
                        previousType.address.type === 'stack'
                            ? RegisterAllocator.RBP
                            : RegisterAllocator.RSP;
                    this.instruction(
                        'addi',
                        `${addressRegister.name},${stackRegister.name},${dataOffset}`
                    );
                    break;
                case 'absolute':
                    const register = previousType.address.register;
            ...
            if (varEntry.type === 'data') {
                ...
                this.instruction(
                    'lw',
                    `${addressRegister.name},${thisEntry.offset}(${RegisterAllocator.RBP.name})`
                );
                this.instruction(
                    'addi',
                    `${addressRegister.name},${addressRegister.name},${this.getDataOffset(
                        varEntry
                    )}`
                );
            } else if (Utils.isReferenceType(varEntry)) {
                this.instruction(
                    'lw',
                    `${addressRegister.name},${varEntry.offset}(${RegisterAllocator.RBP.name})`
                );
            } else {
                const offset = varEntry.offset;
                this.instruction(
                    'addi',
                    `${addressRegister.name},${RegisterAllocator.RBP.name},${offset}`
                );
            }
        }
        ...
        for (const index of node.indices) {
            ...
            this.instruction(
                'add',
                `${addressRegister.name},${addressRegister.name},${indexRegister.name}`
            );
    ...
    private generateFunctionCallExpression(
        ...
        this.instruction(
            'subi',
            `${RegisterAllocator.RSP.name},${RegisterAllocator.RSP.name},${returnTypeSize}`
        );
        ...
        if (previousType) {
            switch (previousType.address.type) {
                ...
                case 'stackTop': {
                    ...
                    this.instruction(
                        'addi',
                        `${register.name},${stackRegister.name},${previousType.address.stackOffset}`
                    );
                    this.push(register);
                    this._registerAllocator.freeRegister(register.id);
                    break;
        ...
        if (arraySizes.length > 1) {
            ...
            for (let dim = 0; dim < arraySizes.length - 1; dim++) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    this.instruction(
                        'addi',
                        `${valueRegister.name},${RegisterAllocator.RBP.name},${
                            varEntry.offset + ptrOffset * 4
                        }`
                    );
                    this.instruction(
                        'addi',
                        `${destRegister.name},${RegisterAllocator.RBP.name},${
                            varEntry.offset + (startOffset + i) * 4
                        }`
                    );
    ...
    private clearAddress(address: Address) {
        if (address.stackSizeToClear) {
            ...
            this.instruction(
                'addi',
                `${RegisterAllocator.RSP.name},${RegisterAllocator.RSP.name},${address.stackSizeToClear}`
            );
            this.line();
        }
            ...
            case 'register':
                switch (dstAddress.type) {
                    case 'register':
                        this.instruction(
                            'add',
                            `${dstAddress.register.name},${srcAddress.register.name},R0`
                        );
                        break;
                    case 'absolute':
                        this.instruction(
                            'sw',
                            `0(${dstAddress.register.name}),${srcAddress.register.name}`
                        );
                        break;
                    case 'stack':
                        this.instruction(
                            'sw',
                            `${dstAddress.stackOffset}(${RegisterAllocator.RBP.name}),${srcAddress.register.name}`
                        );
                        break;
                }
                break;
            case 'absolute':
                switch (dstAddress.type) {
                    case 'register':
                        this.instruction(
                            'lw',
                            `${dstAddress.register.name},0(${srcAddress.register.name})`
                        );
                        break;
                    case 'absolute':
                        this.absoluteMove(
                        ...
                        break;
                    case 'stack':
                        this.absoluteStackMove(
                            srcAddress.register,
                            dstAddress.stackOffset,
                            dstAddress.size
                        );
                        break;
                }
            ...
            case 'stack':
                switch (dstAddress.type) {
                    case 'register':
                        this.instruction(
                            'lw',
                            `${dstAddress.register.name},${srcAddress.stackOffset}(${RegisterAllocator.RBP.name})`
                        );
                        break;
                    case 'absolute':
                        this.stackAbsoluteMove(
                            srcAddress.stackOffset,
                            dstAddress.register,
                            dstAddress.size
                        );
                        break;
            ...
            case 'stackTop':
                switch (dstAddress.type) {
                    case 'register':
                        this.instruction(
                            'lw',
                            `${dstAddress.register.name},${srcAddress.stackOffset}(${RegisterAllocator.RSP.name})`
                        );
                        break;
                    case 'absolute':
                        this.stackAbsoluteMove(
    ...
    private stackMove(
        ...
        dstOffset: number,
        size: number,
        srcStackRegister: Register = RegisterAllocator.RBP,
        dstStackRegister: Register = RegisterAllocator.RBP
    ) {
        const register = this._registerAllocator.allocateRegister();
        let index = 0;
        while (index < size) {
            this.instruction(
                'lw',
                `${register.name},${srcOffset + index}(${srcStackRegister.name})`
            );
            this.instruction(
                'sw',
                `${dstOffset + index}(${dstStackRegister.name}),${register.name}`
            );
            index += 4;
        }
        this._registerAllocator.freeRegister(register.id);
    ...
    private stackAbsoluteMove(
        srcOffset: number,
        dstRegister: Register,
        size: number,
        stackRegister: Register = RegisterAllocator.RBP
    ) {
        const register = this._registerAllocator.allocateRegister();
        let index = 0;
        while (index < size) {
    ...
    private loadInRegister(address: Address): Register {
        switch (address.type) {
            ...
            case 'stack': {
                ...
                this.instruction(
                    'lw',
                    `${register.name},${address.stackOffset}(${RegisterAllocator.RBP.name})`
                );
                this.clearAddress(address);
                return register;
            }
            case 'stackTop': {
                ...
                this.instruction(
                    'lw',
                    `${register.name},${address.stackOffset}(${RegisterAllocator.RSP.name})`
                );
                this.clearAddress(address);
                return register;
            }
    ...
    private loadReferenceInRegister(address: Address): Register {
        switch (address.type) {
            ...
            case 'stack': {
                ...
                this.instruction(
                    'add',
                    `${register.name},${RegisterAllocator.RBP.name},${address.stackOffset}`
                );
                return register;
            }
            case 'stackTop': {
                ...
                this.instruction(
                    'add',
                    `${register.name},${RegisterAllocator.RSP.name},${address.stackOffset}`
                );
                return register;
            }
        }
    ...
    private getDataOffset(varEntry: SymbolTableEntry, classEntry?: SymbolTableEntry) {
        if (!classEntry) classEntry = this.getCurrentClassEntry();

        if (classEntry.symbolTable === varEntry.parentTable) {
            return varEntry.offset;
        }

        const inheritancePath = this.findInheritancePath(
            classEntry,
            varEntry.parentTable.getParentEntry()
        );
        let offset = 0;
    ...
    private findInheritancePath(
        derivedClassEntry: SymbolTableEntry,
        baseClassEntry: SymbolTableEntry,
        inheritancePath: SymbolTableEntry[] = []
    ) {
        inheritancePath.push(derivedClassEntry);

        if (derivedClassEntry === baseClassEntry) return inheritancePath;

        const baseClassEntries = derivedClassEntry.inheritanceList.map((b) =>
            derivedClassEntry.symbolTable.lookup(b)
        );
