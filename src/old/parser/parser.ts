...
function main() {
    ...
    // Grammar
    const grammar: Grammar = options.grammar
        ? GrammarFactory.fromFile(options.grammar)
        : GrammarFactory.fromString(DefaultGrammar);
    ...
    if (!grammarParser.isLL1()) {
        ...
        console.log(
            'Ensure that the supplied grammar is in LL(1) form, without any left recursions or ambiguities.'
        );
        return;
    }
    ...
    const outputErrorFilePath =
        options.input?.replace(/\.[^/.]+$/, '.outsyntaxerrors') || '.outsyntaxerrors';
    ...
    const outputFilePath =
        options.output || options.input?.replace(/\.[^/.]+$/, '.outderivation') || '.outderivation';
    ...
    // Print derivation
    GrammarUtils.printDerivationTree(root, (str) => fs.writeSync(outputFile, str));
