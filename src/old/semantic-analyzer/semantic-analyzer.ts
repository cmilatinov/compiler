...
function main() {
    ...
    if (!options.input && !options['inline-input']) {
        ...
        console.log(`Refer to 'semanticanalyzerdriver --help' for usage description and examples.`);
        return;
    }
    ...
    // Open error file
    const outputErrorFilePath =
        options.input?.replace(/\.[^/.]+$/, '.outsemanticerrors') || '.outsemanticerrors';
    ...
    const derivation = options.input
        ? grammarParser.parseFile(options.input, printErr)
        : grammarParser.parseString(options['inline-input'], printErr);
    if (!derivation) {
        console.log(
            `\n[ERROR]: The input source file or string contains one or more syntax errors.\n`
        );
        console.log(
            'Modify the source code to fix the aforementioned syntax errors and attempt to validate it again.'
        );
        return;
    }
    ...
    const outputFilePath =
        options.output ||
        options.input?.replace(/\.[^/.]+$/, '.outsymboltables') ||
        '.outsymboltables';
    ...
    if (!validator.validate(ast)) {
        ...
        console.log(
            'Modify the source code to fix the aforementioned semantic errors and attempt to validate it again.'
        );
    }

    // Print symbol tables
    Utils.printSymbolTable(validator.getGlobalSymbolTable(), (str) =>
        fs.writeSync(outputFile, str)
    );
