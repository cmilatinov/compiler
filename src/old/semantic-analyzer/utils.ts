...
export function functionFullEquals(func1, func2) {
    ...
    return (
        ...
        _.isEqual(
            func1.parameters.map((p) => ({
                name: p.name,
                varType: p.varType,
                arraySizes: p.arraySizes
            })),
            func2.parameters.map((p) => ({
                name: p.name,
                varType: p.varType,
                arraySizes: p.arraySizes
            }))
...
export function typeEquals(type1, type2) {
    ...
    if (
        type1.varType === integerType.varType &&
    ...
    return type1.varType === type2.varType && arraySizesEquals(type1.arraySizes, type2.arraySizes);
}

function arraySizesEquals(arraySizes1: any[], arraySizes2: any[]) {
    if (arraySizes1.length !== arraySizes2.length) return false;
...
export function stringifyFunction(func: ASTNode) {
    ...
    return `${visibility}${func.name}(${func.parameters
        .map((p) => `${p.name}: ${stringifyType(p)}`)
        .join(', ')}) -> ${func.returnType}`;
}

export function stringifyType(type) {
    if (!type) return 'undeclared';
...
export function isLValue(expression) {
    ...
    while (expression) {
        if (expression.type === 'FunctionCallExpression') return false;
...
export function printSymbolTable(table, print: StringProcessor = DEFAULT_PROCESSOR) {
    ...
    table.getEntries().forEach((e) => {
        if (e.symbolTable) printSymbolTable(e.symbolTable, print);
    });
}
