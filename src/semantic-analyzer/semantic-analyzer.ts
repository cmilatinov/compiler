...
function main() {
    ...
    // Open error file
    const outputErrorFilePath = options.input?.replace(/\.[^/.]+$/, '.outsemanticerrors') || '.outsemanticerrors';
    ...
    // Open output file
    const outputFilePath = options.output || options.input?.replace(/\.[^/.]+$/, '.outsymboltables') || '.outsymboltables';
    ...
    // Print symbol tables
    Utils.printSymbolTable(validator.getGlobalSymbolTable(), str => fs.writeSync(outputFile, str));
